...
/* 
* This function can be used to get the IP and port info of all nodes in a particular group. You can then use
* comm.send to talk to these nodes.
*/
...
groups.put = function(config, group, callback) {
    ...
    distribution[config] = {};
    distribution[config].status = require('../all/status')({gid: config});
    distribution[config].comm = require('../all/comm')({gid: config});
    distribution[config].gossip = require('../all/gossip')({gid: config});
    distribution[config].groups = require('../all/groups')({gid: config});
    distribution[config].routes = require('../all/routes')({gid: config});
    distribution[config].mem = require('../all/mem')({gid: config});
    distribution[config].store = require('../all/store')({gid: config});
    return;
};
