...
const printMerged = (err, data) => {
  ...
  const globalString = JSON.stringify(local, null, 2); // The `null, 2` adds indentation for readability

  // Write the serialized global object to a file
  fs.writeFileSync('helpme.txt', globalString, 'utf8', (err) => {
      if (err) throw err;
      console.log('Data has been written to output_file.json');
  });
  ...
  for (const line of globalIndexLines) {
    ...
    if (urlFreqData != null) {
      ...
      for (let i = 0; i < urlFreqPairs.length; i += 2) {
        ...
        const freq = Number(urlFreqPairs[i + 1], 10);
        urlFreqList.push({url, freq}); // Array of {url, tfIDF} objects
      }
    }
  ...
  for (const term in local) {
    const docsContainingTerm = Object.keys(local).filter(key => key === term).length;
    const tfIDF = Number(local[term].freq) * Math.log10(corpSize / docsContainingTerm); 
    if (term in global) {
      global[term].push({url: local[term].url, freq: tfIDF});
    } else {
      global[term] = [{url: local[term].url, freq: tfIDF}];
    }
  }
  for (const term in local) {
